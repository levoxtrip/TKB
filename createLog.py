...
with open(log_file_path, "w", encoding="utf-8") as log:
    ...
    for file in markdown_files:
        ...
        with open(file, "r", encoding="utf-8") as f:
            ...
            headline_match = re.search(r"^(#{1,6})\s*(.+)", content, re.MULTILINE)
            if headline_match:
                headline = headline_match.group(2).strip()  # Capture only the headline text (group 2)
                print(f"Captured headline: {headline}")
            else:
                headline = "No headline found"
                print(f"No headline found in file: {file}")
